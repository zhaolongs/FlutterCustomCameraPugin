	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.ll_base_back).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				CameraOpacityShowActivity.this.sendBroadcast(lIntent);
		...
		findViewById(R.id.ll_base_next).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (flagTime < 2000 || !mImageLoading) {
					Toast.makeText(CameraOpacityShowActivity.this, "图片正在加载中", Toast.LENGTH_SHORT).show();
				} else {
					if (mIsCorpBoolean) {
						final Intent lIntent = new Intent(CameraOpacityShowActivity.this, CameraExampCorpActivity.class);
						lIntent.putExtra("imageUrl", mImageUrl);
						lIntent.putExtra("mCropHeight", mCropHeight);
						lIntent.putExtra("mCropWidth", mCropWidth);
						CameraOpacityShowActivity.this.startActivity(lIntent);
					} else {

						Toast.makeText(CameraOpacityShowActivity.this, "已保存 " + mImageUrl, Toast.LENGTH_LONG).show();
